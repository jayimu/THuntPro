    ...
    def malsharedown(self, filehash):
        ...
        try:
            ...
            if (b'Sample not found by hash' in malresponse3.content):
                ...
                raise Exception("Sample not found by hash")
            else:
                ...
                while os.path.exists(outputpath):
                    name, ext = os.path.splitext(original_outputpath)
                    if not ext:
                        ext = ''
                    outputpath = f"{name}({counter}){ext}"
                    counter += 1
            ...
            raise Exception("BrokenPipeError or IOError")
