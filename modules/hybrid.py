...
class HybridAnalysisExtractor():
    ...
    def downhash(self, filehash, user_agent='Falcon Sandbox'):
        ...
        try:
            ...
            # 下载二进制内容：优先使用 octet-stream；有些环境返回 gzip
            ...
            try:
                ...
                rc = str(hatext)
                if 'message' in rc or haresponse.status_code >= 400:
                    # 回退方案：搜索该 hash 的多个分析，随机挑选一个环境尝试下载
                    # 改为 JSON 接受头再进行搜索
                    requestsession.headers.update({'accept': 'application/json'})
                    search_url = '/'.join([haurl, 'search', 'hash']) + f'?hash={resource}'
                    sr = requestsession.get(url=search_url)
                    try:
                        js = sr.json() if sr.status_code == 200 else {}
                    except Exception:
                        js = {}
                    reports = (js.get('reports') or []) if isinstance(js, dict) else []
                    # 仅选择可下载的报告（样本共享/可用）
                    candidates = []
                    for r in reports:
                        ...
                        if not env:
                            continue
                        shared = bool(r.get('is_shared') or r.get('sample_available') or r.get('submit_type') == 'public')
                        candidates.append((env, shared))
                    env_ids = [env for env, shared in candidates if shared]
                    if not env_ids:
                        final = 'Malware sample is not available to download (not shared by submitter).'
                        if (cv.bkg == 1):
                        ...
                        return final
                    # 优先常用环境，随后随机
                    priority = [100, 110, 120, 200, 300]
                    env_ids = list(dict.fromkeys(priority + env_ids))
                    success = False
                    for env in env_ids:
                        # 方案A：基于 report 的 sample 下载
                        try_urls = [
                            '/'.join([haurl, 'report', f'{resource}:{env}', 'sample']),
                            # 有的需要使用 report 的 artifacts 下载端点
                            '/'.join([haurl, 'report', f'{resource}:{env}', 'artifacts', 'sample']),
                            '/'.join([haurl, 'overview', resource, 'sample'])
                        ]
                        for u in try_urls:
                            # 切回二进制下载 Accept
                            requestsession.headers.update({'accept': 'application/octet-stream'})
                            rr = requestsession.get(url=u, allow_redirects=True)
                            if rr.status_code < 400 and 'message' not in (rr.text or ''):
                                outputpath = os.path.join(cv.output_dir, f'{resource}.gz')
                                open(outputpath, 'wb').write(rr.content)
